...
do
	...
	echo "cat /proc/meminfo:" | tee -a $TEMP_LOG_FILE
	adb shell cat /proc/meminfo | tee -a $TEMP_LOG_FILE
	echo "dumpsys meminfo:" | tee -a $TEMP_LOG_FILE
	adb shell dumpsys meminfo | tee -a $TEMP_LOG_FILE
	echo "--------------------------------------------------------------------" | tee -a $TEMP_LOG_FILE
	sleep 2s
	let "index++"
done
