...
public:
    ...
    DLGITEMINFO m_ItemInfo[ITEM_COUNT];

	/* write status */
	BOOL m_bStarted;
	BOOL m_bRunning;
	BOOL m_bUserStop;
	int m_CurOp; /* save current operate status */
	...
	BOOL GetItemFromBinFile(int id, char *pBuf, int iLen);
	CString GetItemFromFile(int id);
	BOOL GetItem(int id);
    BOOL HexStrToByte(const int id, const TCHAR *str, int nLen);
    BOOL StrToByte(const int id, const TCHAR *str, const int nLen, const int ext = -1);
...
public:
	...
	/*afx_msg void OnBnClickedCheckCust2();*/
	afx_msg void OnBnClickedCheckCust1();
	afx_msg void OnEnSetfocusEditCust1();
	//afx_msg void OnEnSetfocusEditCust2();
};
