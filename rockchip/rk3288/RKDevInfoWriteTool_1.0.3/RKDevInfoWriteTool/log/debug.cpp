...
void _cdecl CLogger::PrintMSGW(DEBUG_LEVEL dlLevel,const TCHAR *format,...)
{
	...
	if (dlLevel < m_curlevel)
		return;
	if (m_namew.empty())
		return;
	lock();
	buffer_size = dim(m_bufw);
	GetLocalTime( &LocalTime );
	iLength = _snwprintf(m_bufw, buffer_size, TEXT("%02d:%02d:%02d   "),
						 LocalTime.wHour, LocalTime.wMinute, LocalTime.wSecond );

	va_start(args, format);
	iLength += _vsnwprintf(&m_bufw[iLength], buffer_size - iLength, format, args);
	va_end(args);
...
void _cdecl CLogger::PrintMSGA(DEBUG_LEVEL dlLevel,const CHAR *format,...)
{
	...
	if (m_namew.empty())
		return;
	lock();
	buffer_size = dim(m_bufa);
	GetLocalTime(&LocalTime);
	iLength = sprintf_s(m_bufa, buffer_size,"%02d:%02d:%02d   ", LocalTime.wHour, LocalTime.wMinute, LocalTime.wSecond );
	va_start(args, format);
	iLength += vsprintf_s(&m_bufa[iLength], buffer_size - iLength, format, args);
	va_end(args);
	if (m_bufa[iLength] != '\n') {
		m_bufa[iLength++] ='\r';
		m_bufa[iLength++] ='\n';
